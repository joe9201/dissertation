...
def run_fci_algorithm(data, labels):
    ...
    print("Labels passed to FCI algorithm:", labels)
    print("Data shape passed to FCI algorithm:", data.shape)
    print("Data sample:", data[:5])

    if 'absences' not in labels:
        print("Error: 'absences' column is missing in the labels.")
        return None
